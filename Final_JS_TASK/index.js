...
// helper function to avoid escaping single quotes
      ...
      .map(
        (d) =>
          `<div class="max-w-sm rounded-xl overflow-hidden mx-1 mt-5 border-[#ECECEC] border">
                <img onclick="getProductId(${
                  d.id
                })" class="w-full px-2 py-2 items-center justify-center" src="${
            d.images[0]
          }" alt="Sunset in the mountains">
                <div class="px-3 py-4">
                    <div class="font-bold text-sm mb-2">${d.title}</div>
                    <p class="text-[#AEAEAE] text-sm font-bold mt-1">${
                      d.category
                    }</p>
                    <p class="text-[#3BB77E] font-medium mt-1">${d.brand}</p>
                    <p class="text-[#AEAEAE] flex text-sm font-medium mt-1">${
                      d.rating
                    }<span><img src="assets/ratings.svg"></span></p>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center justify-center mt-1 gap-1">
                            <span class="text-[#3BB77E] text-xl underline font-bold">$${(
                              d.price *
                              (1 - d.discountPercentage / 100)
                            ).toFixed(2)}</span>
                            <span class="text-[#ADADAD] text-sm line-through font-semibold ">$${
                              d.price
                            }</span>
                        </div>
                        <div class="flex items-center justify-center">
                            <button onclick='addtocart(${toAttr(d)})'  class="bg-[#DEF9EC] flex py-1 px-1 items-center rounded-sm font-bold text-[#3BB77E] cursor-pointer">
...
async function productListData1() {
  ...
  try {
    ...
    const products = data.products;
    console.log(products);
    productList1.innerHTML = products
      .map(
        (d) =>
          `<div class="w-64 rounded-xl border-[#ECECEC] border flex-shrink-0">
                    <img class="w-full px-2 py-2 items-center justify-center" src="${
                      d.images[0]
                    }">
                    <div class="px-4 py-2">
                        <h2 class="font-bold text-xl mb-2">${d.title}</h2>
                        
                        <p class="text-[#AEAEAE] text-sm font-medium mt-1">${
                          d.category
                        }</p>
                        <p class="text-[#3BB77E] font-medium mt-1">${
                          d.brand
                        }</p>
                        <p class="text-[#AEAEAE] flex text-sm font-medium mt-1">${
                          d.rating
                        ...
                        <div class="flex items-center justify-center">
                            <button  onclick='addtocart(${toAttr(d)})' 
...
async function productListData2() {
  ...
  try {
    ...
    console.log(products);
    window.allProducts = products;
    productList2.innerHTML = products
      .map(
        (d) =>
          `<div class="max-w-sm rounded-xl overflow-hidden mx-1 mt-5 border-[#ECECEC] border">
                <img onclick="getProductId(${
                  d.id
                })" class="w-full h-40 px-2 py-2 items-center justify-center" src="${
            d.images[0]
          }" alt="img">
                <div class="px-3 py-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-center">
                            <button  onclick='addtocart(${toAttr(d)})'  class="bg-[#DEF9EC] flex py-1 px-1 rounded-sm font-bold text-[#3BB77E] cursor-pointer">
                            <span class="py-1"><img src="assets/addcart.svg"></span>
                            <span class="">Add</span></button>
                        </div>
                    </div>
...
async function categoryListData1() {
  ...
  try {
    ...
    categoryList1.innerHTML = data
      .map(
        (d) =>
          `<h2 onclick="getProductCategory('${
            d.slug
          }')" class="text-xs border border-[#F2F3F4] px-4 py-2 mt-3 mx-2 rounded-sm cursor-pointer    ${
            categorySlug === d.slug && "text-green-500"
          } ">${d.name}</h2>`
      )
      .join("");
  } catch (error) {
...
function displaycart() {
        ...
        `<div class="grid grid-cols-6 items-center justify-center gap-6 mb-5 pb-10 border-b border-b-[#BEBCBD]">
                ...
                <div class="flex items-center justify-center">
                    <div class="flex gap-2 bg-[#F6F6F6] text-sm rounded-xl px-2 py-1">
                        <img onclick="decreaseqty(${i})" class="cursor-pointer w-3" src="assets/minus.svg"><span>${
          d.quantity
        }</span><img onclick="increaseqty(${i})" class="cursor-pointer w-3" src="assets/plus.svg">
                    </div>
                </div>
  ...
  cartinfo.innerHTML = `<div class="mt-5">
            ...
            <div class="max-w-sm bg-[#F4F6FA] px-7 py-5 justify-center">
                <div class="border-b border-b-[#BEBCBD]">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-sm">$${subtotal.toFixed(
                          2
                        )}</span>
                    </div>
                    ...
                    <div class="flex justify-between mt-5 mb-5">
                        ...
                        <span class="font-bold text-sm">$${grandtotal.toFixed(
                          2
                        )}</span>
                    </div>
                </div>
...
async function getcategoryitem() {
  ...
  try {
    ...
    //                 </div>
    //                 <div class="flex items-center justify-center">
    //                     <button onclick='addtocart(${JSON.stringify(d)})' class="bg-[#DEF9EC] flex py-1 rounded-sm font-bold text-[#3BB77E] cursor-pointer">
